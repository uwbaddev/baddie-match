...
test('buildWinRankings sorts by event win percentage and includes win-loss text', () => {
  const rankings = buildWinRankings([
    { id: 1, name: 't_Liam Zhang', singles_wins: 2, singles_losses: 2 },
    { id: 2, name: 'Ben Singh', singles_wins: 4, singles_losses: 1 },
    { id: 3, name: 'Cyd Tran', singles_wins: 0, singles_losses: 0 },
  ], 'singles');

  expect(rankings).toEqual([
    expect.objectContaining({ id: 2, rank: 1, name: 'Ben Singh', winPct: 80, record: '4:1' }),
    expect.objectContaining({ id: 1, rank: 2, name: 'Liam Zhang', to: '/players/2025-26/liam-zhang', winPct: 50, record: '2:2' }),
    expect.objectContaining({ id: 3, rank: 3, name: 'Cyd Tran', winPct: 0, record: '0:0' }),
  ]);
});
...
test('makeProfileStats ranks win-rate stats by id and normalized names', () => {
  const stats = [
    ...
    { id: 2, name: 'Allison Cheng', singles_wins: 8, singles_losses: 1, doubles_wins: 0, doubles_losses: 2, mixed_wins: 5, mixed_losses: 1 },
    { id: 3, name: 'Ben Singh', singles_wins: 2, singles_losses: 0, doubles_wins: 4, doubles_losses: 0, mixed_wins: 0, mixed_losses: 0 },
  ];

  expect(makeProfileStats(stats[0], stats)).toEqual([
    expect.objectContaining({ label: 'SINGLES', rank: 3, wins: 4, losses: 1, winPct: 80 }),
    expect.objectContaining({ label: 'DOUBLES', rank: 2, wins: 1, losses: 1, winPct: 50 }),
    expect.objectContaining({ label: 'MIXED', rank: 3, wins: 0, losses: 1, winPct: 0 }),
    expect.objectContaining({ label: 'OVERALL', rank: 3, wins: 5, losses: 3, winPct: 63 }),
  ]);
});
  ...
  const doublesRankings = buildEloRankings([
    {
      ...
      doubles_games_played: 4,
      doubles_win_pct: 0.5,
    },
    {
      name: 'Liam Zhang',
      doubles_rating: { mu: 27.4, sigma: 3.2 },
      doubles_games_played: 2,
      doubles_win_pct: 0.25,
    },
  ], 'doubles');

  expect(makeProfileEloCards({ id: 113, name: 't_Liam Zhang' }, singlesRankings, doublesRankings)).toEqual([
    expect.objectContaining({ label: 'Singles Elo', rank: 1, mu: 31.4, sigma: 2.8, games: 3, winPct: 67 }),
    expect.objectContaining({ label: 'Doubles Elo', rank: 2, mu: 27.4, sigma: 3.2, games: 2, winPct: 25 }),
  ]);
});
